...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let greeting_handle = wire
        .call(ProcedureIndex::new(0), ("John Doe".to_string(),))
        .await?;
    wire.flush_end(&mut stream).await?;
    wire.fill(&mut stream).await?;
    let greeting: String = greeting_handle.wait().await?;
